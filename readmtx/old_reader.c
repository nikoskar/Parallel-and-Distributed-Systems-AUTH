...
int *printsliced_k (uint32_t start,
                    uint32_t end,
                    uint32_t j,
                    uint32_t *csc_col,
                    uint32_t *csc_row,
    ...
    for (int i = 0; i < N-2; i++) {
      ...
      if (sliced[0] > i) {
        foreach(int *j, sliced) {
          ...
          int val = *j;
          uint32_t start2 = csc_col[val];
          uint32_t end2 = csc_col[val+1];

          int *sliced2 = printsliced_k(start2, end2, *j, csc_col, csc_row, N);
          int size2 = sizeof(sliced2)/sizeof(sliced2[0]);
          printf("\n\n");

          printf("******currently in loop j: %d", val);
          ...
          if(sliced2[0] > val) {
            ...
            foreach (int *k, sliced2) {
              ...
              uint32_t start3 = csc_col[*k];
              uint32_t end3 = csc_col[*k+1];

              int *sliced3 = printsliced(start3, end3, *k, csc_col, csc_row, N);
              int size = sizeof(sliced)/sizeof(sliced[0]);
